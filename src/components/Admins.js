...
const Admins = ({ data, setPerPage, prev, next, per_page, page, totalUsers, loading, handlePCMSearch, filterPCMByStatus, pcmSearchText }) => {
    ...
    <div className="users-list-container">
      <div className="top d-flex align-items-center justify-content-between">
        <div className='d-flex align-items-center mr-5'>
          <p className="mx-3 fw-bold">No of Admins </p>
          <span className="badge bg-green text-white">{data.length}</span>
        </div>
        <div className='search-bar-container'>
          <i className="fa fa-search" aria-hidden="true"></i>
          <input onChange={handlePCMSearch} value={pcmSearchText} type="text" className="form-control flex-grow-1" placeholder='Search for Admin' />
        </div>
              ...
              <tbody>
                {
                  data.map(x => (
                    <tr key={x.id} className={x.active ? 'active pointer' : 'pointer'}>
                      ...
                      <td >{x.email}</td>
                      <td >{x.occupation}</td>
                      <td >{moment(x.created_at).format("DD-MM-YYYY hh:MMa")}</td>
                    </tr>
                  ))
                }
